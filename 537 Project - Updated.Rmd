...
cox_adj <- coxph(agvhd_surv ~ mtx + age + male + cmv + as.factor(disgroup) + waittime + as.factor(hospital), data = bmt)
...
## multivariable Cox for dfs
cox_dfs_adj <- coxph(dfs_surv ~ deltap + age + as.factor(hospital) + male, data = bmt)
...
## multivariable Cox for relapse
cox_relapse_adj <- coxph(relapse_surv ~ deltap + age + as.factor(hospital) + male, data = bmt)
multi_relapse <- tidy(cox_dfs_adj, conf.int = T, exponentiate = T)
