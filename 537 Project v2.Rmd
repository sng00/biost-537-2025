---
title: "BIOST 537 Final Group Project"
author: "Cecilia Martindale, Sarah Garcia, Emily Li"
date: "`r Sys.Date()`"
output:
...
```{r}
## add time-varying deltaa for dfs
...
## univariate Cox for dfs with time-varying deltaa
cox_dfstime <- coxph(Surv(tstart, tdfs, deltadfs) ~ agvhd_tv + cluster(id), data = tdata_dfs)
summary(cox_dfstime)

## multivariate Cox for dfs with time-varying deltaa
...
## add time-varying deltaa for relapse
...
## univariate Cox for relapse with time-varying deltaa
cox_retime <- coxph(Surv(tstart, tdfs, deltar) ~ agvhd_tv + cluster(id), data = tdata_re)
summary(cox_retime)

## multivariate Cox for relapse with time-varying deltaa
...
## add time-varying deltap for dfs
tdata_dfs <- tmerge(data1 = bmt,
             data2= bmt,
             id = id,
             event = event(tdfs, deltadfs),
...
## univariate Cox for relapse with time-varying deltap
cox_retime <- coxph(Surv(tstart, tdfs, deltar) ~ deltap_tv + cluster(id), data = tdata_re)
summary(cox_retime)
